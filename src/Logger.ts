...
class Logger implements ILogger {
  ...
  private constructor(_logger: ILogger) {
    ...
    this.monitorWorker = new Worker(new URL('./monitor.worker.ts', import.meta.url).href, {
      type: 'module',
    })
    this.downloadWorker = new Worker(new URL('./download.worker.ts', import.meta.url).href, {
      type: 'module',
    })
    this.getLogSize()
  }
  ...
  format(level: LogLevel, message: string): string {
    ...
    }: ${message.replace(/(?<=data:image\/\w+;base64,)(.*?)(?=")/, '<no record>')}`
      .replaceAll('\n', '')
      .trim()
  }
  ...
  info(message: string) {
    this._logger.log(LogLevel.Information, this.format(LogLevel.Information, message))
  }
  warn(message: string) {
  ...
  async getLogSize() {
    this.monitorWorker.onmessage = event => {
      const { totalSizeInMB, count, totalCompressedSizeInMB, error } = event.data
      if (error) {
        console.error(error)
        this.error(error)
      }
