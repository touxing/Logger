...
export class IndexedDBClient {
  ...
  // 创建不同的客户端实例
  public static createInstance(options: { dbName: string; storeName: string }): IndexedDBClient {
  ...
  public async open(): Promise<IDBDatabase> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = event => {
        reject(`open database failed: ${(event.target as IDBOpenDBRequest).error}`)
      }
      request.onsuccess = () => {
  ...
  public async create(data: any): Promise<void> {
    return this.executeTransaction('readwrite', store => {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = resolve
        request.onerror = event => reject(`insert failed: ${(event.target as IDBRequest).error}`)
      })
    })
  }
  ...
  public async find(id: number): Promise<any> {
    return this.executeTransaction('readonly', store => {
      return new Promise((resolve, reject) => {
        ...
        request.onerror = event => reject(`find failed: ${(event.target as IDBRequest).error}`)
      })
    })
  }
  ...
  public async update(data: { id: number; [key: string]: any }): Promise<void> {
    return this.executeTransaction('readwrite', store => {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = resolve
        request.onerror = event => reject(`update failed: ${(event.target as IDBRequest).error}`)
      })
    })
  }
  ...
  public async remove(id: number): Promise<void> {
    return this.executeTransaction('readwrite', store => {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = resolve
        request.onerror = event => reject(`delete failed: ${(event.target as IDBRequest).error}`)
      })
    })
  }
    ...
    return this.executeTransaction('readwrite', store => {
      ...
      return new Promise((resolve, reject) => {
          ...
          const deletePromises = keysToDelete.map(key => {
            return new Promise((delResolve, delReject) => {
              ...
              delRequest.onerror = event =>
                delReject(`Delete failed: ${(event.target as IDBRequest).error}`)
            })
          })
          try {
        ...
        allRecords.onerror = event => {
          reject(`Fetch all records failed: ${(event.target as IDBRequest).error}`)
        }
      })
    })
  }
  ...
  public async findAll<T = any>(): Promise<T[]> {
    return this.executeTransaction('readonly', store => {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onerror = event => reject(`Find all failed: ${(event.target as IDBRequest).error}`)
      })
    })
  }
      ...
      return this.executeTransaction('readonly', store => {
        return new Promise((resolve, reject) => {
          ...
          cursorRequest.onerror = event =>
            reject(`Find all chunk failed: ${(event.target as IDBRequest).error}`)
        })
      })
    }
